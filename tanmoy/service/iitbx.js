...
module.exports = {
	...
	deleteParticularObjectAPI : function(req,res){
		...
		request(requestParameter, function(error, response, body) {
			res.status(200).send(req.params['obj_name'] + " Deleted")
		});
	...
	createCourseAPI : function(req,res){
		...
		course_name = course_name.replace(/ /g,'_');
	...
	deleteCourseAPI : function(req,res){
		...
		request(requestParameter, function(error, response, body) {
			...
			{  	
					var requestParameter = {
						uri: storageUrl + req.params['course_name']+ "/" + objects[i],
						method: "DELETE",
						headers : {
							'content-type' : 'application/json'
						}
